...
String buildControlPage() {
  ...
  page += F("button{width:100%;height:72px;border:0;border-radius:8px;background:#1f6feb;color:white;font-size:18px;font-weight:700;}");
  ...
  page += F("<div class='empty'></div><button onclick=\"sendCmd('/forward','Forward')\">Forward</button><div class='empty'></div>");
  page += F("<button onclick=\"sendCmd('/left','Left')\">Left</button><button class='stop' onclick=\"sendCmd('/stop','Stop')\">Stop</button><button onclick=\"sendCmd('/right','Right')\">Right</button>");
  page += F("<div class='empty'></div><button onclick=\"sendCmd('/back','Back')\">Back</button><div class='empty'></div>");
  page += F("</div>");

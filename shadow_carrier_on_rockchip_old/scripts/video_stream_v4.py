#!/usr/bin/env python3
"""video_stream_v4.py - 修复stdout污染 (v4)
v3 -> v4 改动 (修复花屏/卡顿/死锁根因):
  v3 bug: daemon的stdout被rknn库调试printf污染, 与JSON混在一起
    → Python readline读到printf行(json解析失败, infer=0ms)
    → 没等daemon推理完就去读out.jpg → 读到半帧 → 一半灰
    → latest_jpeg更新慢 → 推流卡顿
    → 管道积压满 → 死锁冻结
  v4 修复:
    daemon: stdout→/dev/null(丢库printf), JSON改走stderr隔离
    Python: stderr=PIPE读JSON(过滤{开头行), stdout=DEVNULL
"""
...
def start_daemon():
    global daemon
    # v4: stdout丢弃(库printf污染), stderr传JSON(隔离)
...
def infer(path):
    """从daemon stderr读JSON, 过滤调试行(非{开头)"""
